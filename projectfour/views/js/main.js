...
/*
 Optimize CSS Delivery: bootstrap-grid.css is loaded after the initail painting of the page.
 After that, styles are applied to the page once it finishes loading; the initial render of the crtical
...
var cb = function() {
  ...
  heading.parentNode.insertBefore(l, heading);
};
...
// replaced misspelled array element 'noise' to 'noisy'
...
var pizzaElementGenerator = function(i) {
  ...
  var documentFrag = document.createDocumentFragment(); // create empty document fragment
  ...
  documentFrag.appendChild(pizzaContainer);
...
var resizePizzas = function(size) { 
  ...
  // changed querySelector to getElementById, less error prone
  ...
  function changePizzaSizes(size) {
    var randPizzaBox = document.querySelector(".randomPizzaContainer");
    var dx = determineDx(randPizzaBox, size);
    var newwidth = (randPizzaBox.offsetWidth + dx) + 'px';
    var selectors = document.querySelectorAll(".randomPizzaContainer");

    // added a reverse for-loop to loop through random pizza and change width
    for (var i = selectors.length; i--;) {
      selectors[i].style.width = newwidth;
    }
  }
...
// creates pizza using DOM Nodes and appends as the page loads
document.addEventListener("DOMContentLoaded", function(event) { 
  // empty DOM node that is not part of the main DOM tree
  var docFragment = document.createDocumentFragment();

  var newPizza = document.getElementById("randomPizzas");
  for (var i = 2; i < 100; i++) {
    // append new pizza
    docFragment.appendChild(pizzaElementGenerator(i));
  }
  newPizza.appendChild(docFragment);
});
...
function updatePositions() {
  ...
  var topScrol = (document.body.scrollTop / 1250); 
  
  // reverse loop for faster performance
  for (var i = items.length; i--;) {
     var phase = Math.sin(topScrol + (i % 5));
     var side = -items[i].basicLeft + 100 * phase + 'px'; 
     items[i].style.transform = "translateX("+side+") translateZ(0)";
  }
...
// this runs updatePositions on 'scroll'
